...
> MBR（Mission-Boundary-Resources）の本体テンプレートは SKILL.md Section 3 を参照。
...
- 4 名以上のチーム: Chief of Staff + Red-Team + QA-Manager の 3 名が必須（人数カウントに含む）
...
| リサーチャー | 1番目 | L1 | 調査系 |
| アーキテクト | 2番目（リサーチャー後） | L1 | 調査系 |
| フロントエンジニア | 3番目（並列） | L1 | 実装系 |
| バックエンジニア | 3番目（並列） | L1 | 実装系 |
| Red-Team | 4番目 | L1 | 調査系 |
...
リサーチャー       → 既存コードと要件の調査
アーキテクト       → 設計決定（リサーチャー完了後）
フロントエンジニア ─┐ 並列（担当ディレクトリ分割）
バックエンジニア   ─┘（アーキテクト完了後）
...
|---|---|---|---|
| リサーチャー | 1番目 | L1 | 調査系 |
| テクニカルライター | 2番目 | L1 | 調査系 |
| QA-Manager | 3番目 | L1 | 調査系 |

ドキュメントのみのタスクでは Red-Team を QA-Manager に統合可（例外）。3 名以下の小規模チームでは CoS 不要（Leader が直接集約・報告）。
