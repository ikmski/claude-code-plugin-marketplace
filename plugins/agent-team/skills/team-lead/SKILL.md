...
> 詳細情報: Agent Teams API リファレンスは `reference.md`、会話例は `examples.md` を参照。
...
|------|--------|-----------|------|-------------|
| 実装系 | エンジニア、バグフィクサー等 | `"worktree"` | `"bypassPermissions"` | worktree 内 + `artifacts/{role-name}/` |
...
4. **チーム編成案作成**: 最大 9 名の役割を定義し、各 MBR プロンプトを作成する
5. **編成案提示**:
   - 小〜中規模（6 名以下）: 報告して即実行
...
**思考・探索系**: リサーチャー / アナリスト / アイデアジェネレーター
**行動・実行系**: アーキテクト / 各種エンジニア（フロントエンド、バックエンド、DB、インフラ等）/ テクニカルライター
...
Leader への SendMessage では、`recipient` に `team-lead` を指定します。
...
> Red-Team 専用テンプレートは reference.md を参照。Autonomy Protocol の推奨フレームワーク（任意参照）も reference.md にある。
