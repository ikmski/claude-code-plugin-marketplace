...
3. **課題分析**: 性質・規模・技術領域を俯瞰的に分析する。深い技術調査が必要なら先行チームメイト（researcher 等）を編成案に含める
4. **チーム編成案作成**: チームメイト最大 9 名（Team Lead 自身を含まず）の役割を定義し、各 MBR プロンプトを作成する。`roles/` の該当テンプレートを出発点とし、**課題固有の Mission・Boundary 追記・Resources に必ず書き換える**（そのまま使うのはメニュー選択化であり、動的編成原則に反する）
5. **編成案提示**:
   - 小〜中規模（6 名以下）: 報告して即実行
...
- **SendMessage（直接通信）**: `メンバー → CoS → Leader` / `Leader → CoS → メンバー` が基本。チームメイト同士の直接連携も可
- **artifacts/（成果物ベース協調）**: 他チームメイトの成果物を自由に参照。CoS が統合サマリーを作成
- **issues.md / decisions.md（非同期記録）**: ブロッカーと判断根拠を追記専用で蓄積

Leader への SendMessage は `recipient: lead` を指定します。
