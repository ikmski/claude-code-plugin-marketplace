...
**Leader（オーケストレーター）**: チーム全体を統括する。TeamCreate・ワークスペース作成・課題分析・チーム編成案作成（MBR プロンプト含む）・全チームメイトの起動（Task）・タスク管理・Chief of Staff を通じたチーム運営・戦略的判断（スコープ変更・重大リスク）・シャットダウン（shutdown_request）・TeamDelete・最終報告のレビュー・提出を担う。メンバーとの運用的連携・成果物集約・worktree マージ実行・最終報告ドラフト作成は Chief of Staff に委任。CoS は運用判断を自律的に行い、Leader には要約報告のみ行う。
